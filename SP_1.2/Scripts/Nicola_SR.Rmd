...
#                     pars=c("beta", "alpha", "gamma", "sigma", "surv_est", "nu_Y", "nu_rec", "srep", "smsy_85", "smsy", "umsy"))
# 
# #fit summary
# out <- summary(fit)
...
ricker <- function(x){
  (-out$summary[1,1] * x + out$summary[3,1] + out$summary[2,1])
}

ricker_exp <- function(x){
  exp(-out$summary[1,1] * x + out$summary[3,1] + out$summary[2,1]) * x
}
...
uncertainty_ricker <- apply(as.matrix(poste), 1, function(poste) {
  stat_function(fun=function(x){(poste[1] + poste[2] * x + poste[3])}, colour="blue",
                alpha = 0.02, linewidth = 0.01)
  })

uncertainty_ricker_exp <- apply(as.matrix(poste), 1, function(poste) {
  stat_function(fun=function(x){exp(poste[1] + poste[2] * x + poste[3]) * x}, colour="blue",
                alpha = 0.02, linewidth = 0.01)
  })
